...
function Banner({
  ad, slot, isMain = false,
}: {
  ad?: Ad; slot: number; isMain?: boolean;
}) {
      ...
      {ad && (
        <>
          ...
          >
            ...
            {ad.subText && (
              <p
                className="text-[11px] leading-relaxed mt-2 max-w-[200px]"
                style={{ color: cfg.textLight ? "rgba(255,255,255,0.55)" : "#94A3B8" }}
              >
                {ad.subText}
              </p>
            )}
  ...
  return (
    <section className="w-full py-4">
      ...
      <motion.div
        ...
        className="hidden md:grid gap-4 px-14"
        style={{ gridTemplateColumns: "60% 1fr", gridTemplateRows: "auto" }}
      >
        {/* Slot 1 — large left spanning full height */}
        <div style={{ gridRow: "1 / 3", minHeight: "460px" }}>
          <div className="h-full">
            <Banner ad={getSlot(1)} slot={1} isMain />
          </div>
        </div>
