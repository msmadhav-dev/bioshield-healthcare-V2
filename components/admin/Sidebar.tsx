...
const storeItems = [
  {
    ...
    icon:  Tag,
    sub: [
      { label: "View Categories", href: "/admin/online-store/categories",     icon: Eye        },
      { label: "Add Category",    href: "/admin/online-store/categories/add", icon: PlusCircle },
    ],
  },
  {
        ...
        return (
          <div key={item.label}>
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-[10px] rounded-lg mb-1 text-[13px] font-medium transition-all duration-150 text-left cursor-pointer border-none bg-transparent"
            ...
            {isOpen && (
              <div className="ml-8 mb-2 border-l-2 pl-3" style={{ borderColor: "#E5E7EB" }}>
                {item.sub.map((sub) => {
